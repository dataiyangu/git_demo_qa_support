...
public class TestRedis {
    ...
    public void test1(){
        Jedis redis = null;//连接redis
        try {
            redis = new Jedis("10.0.1.44",6379);
        } catch (Exception e) {
            e.printStackTrace();
        }
//        redis.auth("123.com");//验证密码
        //KEYS
...
//        }
        ...
        redis.set("miao",String.valueOf(System.currentTimeMillis()));
        System.out.println("成功的连接了redis");
        /**  KEY操作

         //KEYS
        et keys = redis.keys("*");//列出所有的key，查找特定的key如：redis.keys("foo")
         Iterator t1=keys.iterator() ;
         while(t1.hasNext()){
         Object obj1=t1.next();
         System.out.println(obj1);
